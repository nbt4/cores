-- ProcurementCore schema and central branding extension.
ALTER TABLE branding_config ADD COLUMN IF NOT EXISTS logo_procurement_sidebar VARCHAR(512);
ALTER TABLE branding_config ADD COLUMN IF NOT EXISTS logo_procurement_login VARCHAR(512);
ALTER TABLE branding_config ADD COLUMN IF NOT EXISTS favicon_procurement VARCHAR(512);
...
CREATE TABLE IF NOT EXISTS proc_suppliers (
 ...
 website VARCHAR(1000), contact_name VARCHAR(160), email VARCHAR(255), phone VARCHAR(80), payment_terms VARCHAR(120),
 default_lead_days INTEGER NOT NULL DEFAULT 0, rating DOUBLE PRECISION NOT NULL DEFAULT 0,
 preferred BOOLEAN NOT NULL DEFAULT FALSE, active BOOLEAN NOT NULL DEFAULT TRUE,
 risk_level VARCHAR(20) NOT NULL DEFAULT 'low', notes TEXT,
...
CREATE TABLE IF NOT EXISTS proc_products (
 id BIGSERIAL PRIMARY KEY, sku VARCHAR(80) NOT NULL UNIQUE, name VARCHAR(240) NOT NULL, description TEXT,
 category_id BIGINT REFERENCES proc_categories(id), unit VARCHAR(30) DEFAULT 'Stk.', manufacturer VARCHAR(180), model VARCHAR(180),
 parameters JSONB NOT NULL DEFAULT '{}', active BOOLEAN NOT NULL DEFAULT TRUE, reorder_point DOUBLE PRECISION DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS proc_price_histories (
 id BIGSERIAL PRIMARY KEY, offer_id BIGINT NOT NULL REFERENCES proc_offers(id) ON DELETE CASCADE,
 price_cents BIGINT NOT NULL, currency VARCHAR(3) NOT NULL, recorded_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS proc_requisitions (
 ...
 status VARCHAR(30) NOT NULL DEFAULT 'draft', requester_id BIGINT NOT NULL, requester_name VARCHAR(160), cost_center VARCHAR(80),
 justification TEXT, needed_by TIMESTAMPTZ, estimated_total_cents BIGINT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS proc_purchase_orders (
 ...
 requisition_id BIGINT REFERENCES proc_requisitions(id), status VARCHAR(30) NOT NULL DEFAULT 'draft', currency VARCHAR(3) NOT NULL DEFAULT 'EUR',
 total_cents BIGINT NOT NULL DEFAULT 0, ordered_by BIGINT NOT NULL, ordered_by_name VARCHAR(160), order_date TIMESTAMPTZ,
 expected_delivery TIMESTAMPTZ, notes TEXT, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
CREATE TABLE IF NOT EXISTS proc_purchase_order_lines (
 ...
 product_id BIGINT REFERENCES proc_products(id), description VARCHAR(500) NOT NULL, quantity DOUBLE PRECISION NOT NULL,
 received_quantity DOUBLE PRECISION NOT NULL DEFAULT 0, unit VARCHAR(30), unit_price_cents BIGINT NOT NULL DEFAULT 0, purchase_url VARCHAR(2000)
);
