-- Central Microsoft Entra identity source and shared M365 application settings.
-- The dashboard also applies these statements idempotently on startup so existing
-- installations do not depend on PostgreSQL's one-time init directory.
...
CREATE TABLE IF NOT EXISTS m365_settings (
    ...
    mailbox_id VARCHAR(255) NOT NULL DEFAULT '',
    sync_interval VARCHAR(32) NOT NULL DEFAULT '5m',
    calendar_mailbox VARCHAR(255) NOT NULL DEFAULT '',
    app_base_url VARCHAR(512) NOT NULL DEFAULT '',
    user_mode VARCHAR(20) NOT NULL DEFAULT 'local',
    user_sync_enabled BOOLEAN NOT NULL DEFAULT FALSE,
    user_group_id VARCHAR(128) NOT NULL DEFAULT '',
    user_sync_interval_minutes INT NOT NULL DEFAULT 60,
    disable_removed_users BOOLEAN NOT NULL DEFAULT TRUE,
    microsoft_login_enabled BOOLEAN NOT NULL DEFAULT FALSE,
    last_user_sync_at TIMESTAMPTZ,
    last_user_sync_status VARCHAR(32) NOT NULL DEFAULT 'never',
    last_user_sync_error TEXT NOT NULL DEFAULT '',
    last_user_sync_count INT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS microsoft_user_profiles (
    ...
    microsoft_id VARCHAR(128) NOT NULL UNIQUE,
    user_principal_name VARCHAR(255) NOT NULL DEFAULT '',
